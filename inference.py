...
def top_k_sampling_decode(encoder_features, model, tokenizer, device, max_length, k=50):
    """
    Generate a caption by top-k sampling from the decoder output.
    ...
    Args:
        ...
        k: Number of tokens to sample from at each step.
...
def nucleus_sampling_decode(
    encoder_features, model, tokenizer, device, max_length, p=0.9
):
    """
    Generate a caption by nucleus sampling from the decoder output.
    ...
    Args:
        ...
        device: Device to move tensors to.
        max_length: Maximum length for the generated caption.
        p: The cumulative probability threshold for selecting tokens.
    ...
    with torch.no_grad():
        for _ in range(max_length - 1):
            ...
            next_token_logits[0, sorted_indices[sorted_indices_to_remove]] = -float(
                "Inf"
            )
...
def beam_search_decode(
    ...
    """
    Generate a caption using beam search from the decoder output.
    ...
    Args:
        ...
        length_penalty: Penalty applied to the length of the generated sequence.
...
def generate_caption_for_image(
    ...
    """
    Generate captions for an image using different decoding strategies.

    Args:
        image_path: Path to the image for which to generate captions.
        model: ImageCaptionModel instance used for encoding and decoding.
        tokenizer: Tokenizer instance for decoding token sequences.
        device: Device to perform computations on (e.g., 'cpu' or 'cuda').
        max_length: Maximum length for the generated captions.
        top_k: If True, use top-k sampling decoding.
        nucleus: If True, use nucleus sampling decoding.
        beam: If True, use beam search decoding.

    Returns:
        A tuple containing captions generated by top-k sampling, nucleus sampling, and beam search decoding strategies.
    """
    ...
    with torch.no_grad():
        ...
        strategies = [
            (top_k, "Top-K Sampling", top_k_sampling_decode),
            (nucleus, "Nucleus Sampling", nucleus_sampling_decode),
            (beam, "Beam Search", beam_search_decode),
        ]

        # Initialize tqdm progress bar (hides time info)
        with tqdm(
            total=sum([s[0] for s in strategies]),
            desc="Generating Captions",
            bar_format="{l_bar}{bar} [{n_fmt}/{total_fmt}]",
        ) as pbar:
            for use_strategy, name, decode_func in strategies:
                if use_strategy:
                    print(f"🔹 Using {name} decoding...")  # Print before each decoding
                    time.sleep(0.5)  # Simulate slight delay (optional)
                    result = decode_func(encoder_features, model, tokenizer, device, max_length=max_length)
                    if name == "Top-K Sampling":
                        caption1 = result
                    elif name == "Nucleus Sampling":
                        caption2 = result
...
def capture_image(image_path, CAMERA_TIMEOUT):
    """
    Captures an image using the Picamera2 and saves it to the specified path.

    Args:
        image_path: The file path where the captured image will be saved.
        CAMERA_TIMEOUT: Time in seconds to wait for the camera to adjust before capturing the image.
    ...
    picam2 = Picamera2()
    picam2.start()
    time.sleep(CAMERA_TIMEOUT)  # Allow the camera to adjust
...
def text_to_speech(text, filename="output.mp3"):
    ...
    Args:
        ...
        filename: The name of the audio file to save the speech. Defaults to "output.mp3".
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "--capture",
        action="store_true",
        help="Capture a new image using the Raspberry Pi camera.",
    )
    parser.add_argument(
        "-t",
        "--timeout",
        type=int,
        default=10,
        help="Time in seconds to wait for the camera to adjust before capturing an image (default: 10s).",
    )
    parser.add_argument(
        "-i",
        "--image",
        type=str,
        help="Path to an existing image for captioning. Required if --capture or -c is not used.",
    )
    parser.add_argument(
        "-m",
        "--model",
    ...
    parser.add_argument(
        "--nucleus",
        action="store_true",
        help="Use nucleus sampling for caption generation.",
    )
    parser.add_argument(
        "--beam", action="store_true", help="Use beam search for caption generation."
    )
    parser.add_argument(
        "--device",
        type=str,
...
def inference():
    ...
    if args.audio:
        if args.topk:
            caption1 = "top-k decoder says " + caption1
            text_to_speech(caption1)
            print("Generated Caption (top-k):", caption1)
        if args.nucleus:
            caption2 = "nucleus decoder says " + caption2
            text_to_speech(caption2)
            print("Generated Caption (nucleus):", caption2)
        if args.beam:
            caption3 = "beam decoder says " + caption3
            text_to_speech(caption3)
